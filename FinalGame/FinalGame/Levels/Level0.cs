...
{
    class Level0 : RC_GameStateParent
    {
        ...
        public override void LoadContent()
        {
            ...
            background = Content.Load<Texture2D>("Textures/p");

            this.backgroundOriginal = Content.Load<Texture2D>("Textures/p_original");
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (levelManager.getGameStatus())
            {
                ...
                if (Utils.TerrainModifier.IntersectPixels(backgroundRectangle, color, bulletRectangle, bulletColor))
                {
                    ...
                    {
                        Utils.TerrainModifier.RebuildTerrain(new Circle(new Vector2(rocketPosition.X + 10, rocketPosition.Y + 10), 55), background, color, Color.White);
                        bulletActive = false;
                        rocketFlying = false;
                        rocketPosition = Vector2.Zero;
                        this.firing = false;
                    ...
                    {
                        Utils.TerrainModifier.DestroyCircle(new Circle(new Vector2(rocketPosition.X + 10, rocketPosition.Y + 10), 35), background, color);
                        bulletActive = false;
                        rocketFlying = false;
                        rocketPosition = Vector2.Zero;
                        this.firing = false;
        ...
        public void shoot()
        {
            ...
            rocketDirection *= levelManager.getCurrentPlayer().getPower();
        ...
        public void simulateGravity()
        {
            foreach (Player player in players)
            {
                if (!player.getParachuteVisibility()) break;
